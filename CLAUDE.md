...
`MenuManager` owns the status item and the snippets menu. The history menu is `FilterMenu`, a custom `NSMenu` with an embedded search field (`TextFieldMenuItem`). `SnippetMenu` has the same field: the item speaks to whatever menu it is in through `FilterableMenu.update(filter:)`, never to a concrete class. The two matchers are deliberately separate — history goes through `ClipFilter` and SQL, while snippets re-filter the `[CPYFolder]` graph already in memory with `SnippetFilter` (a case-insensitive literal substring; the snippet database has no fts index), flattening folder→snippet into one list while a query is active.
